...
class ClipRCNNWithClipBackboneWithOffsetGenTrainable(ClipRCNNWithClipBackbone):
    ...
    def __init__(self, cfg) -> None:
        ...
        self.source_text_token = clip.tokenize("Sunny, Day, Driving").cuda()
        self.di = invariant()
        self.ds = specific()
        self.conv_out = convblock(2 * 1024, 1024, 3, 1, 1)
        self.consistency_loss = ConsistencyLoss()
        self.style_align_proj = nn.Linear(1024, 512)
        self.source_text_token = clip.tokenize("Sunny, Day, Driving")
    ...
    def forward(self, batched_inputs):
        ...
        if not hasattr(self, 'source_text_token'):
            # Fallback 防止报错，实际请在 init 中定义
            import clip
            self.source_text_token = clip.tokenize("Sunny, Day, Driving")
        tokens = self.source_text_token.to(base_ds.device)

        with torch.no_grad():
            # 获取 CLIP 文本特征 (E_text)
            clip_model = self.roi_heads.box_predictor.cls_score.model
            source_text_embed = clip_model.encode_text(tokens)
            source_text_embed = source_text_embed / source_text_embed.norm(dim=-1, keepdim=True)

        # 获取图像风格特征 (Fs) 并进行全局平均池化 + 归一化
        # base_ds: (N, C, H, W) -> (N, C)
        style_feat_flat = F.adaptive_avg_pool2d(base_ds, (1, 1)).flatten(1)
        style_feat_norm = style_feat_flat / (style_feat_flat.norm(dim=-1, keepdim=True) + 1e-6)

        # 计算 Cosine Similarity
        # (N, C) * (1, C).T -> (N, 1)
        sim_sc = torch.matmul(style_feat_norm, source_text_embed.t())
        loss_sc_val = 1.0 - sim_sc.mean()
        ...
        losses.update({'loss_source_style_consistency': loss_sc_val})
    ...
    def opt_offsets(self, batched_inputs):
        ...
        for i, tokens in enumerate(self.cot_tokens):
            ...
            f_mean, f_std = calc_mean_std(features['res4'])
            normalized_feat = (features['res4'] - f_mean) / f_std
            aug_feat = normalized_feat * current_std + current_mean
